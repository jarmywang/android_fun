...
public class PullScrollView extends ScrollView {
    /** 阻尼系数,越小阻力就越大. */
    private static final float SCROLL_RATIO = 0.40f;

    /** 滑动至翻转的距离. */
    private static final int TURN_DISTANCE = 100;

    /** 头部view. */
    private View mHeader, mHeaderS;
    ...
    private int mInitTopS, mInitBottomS;

    /** 头部图片拖动时顶部和底部. */
    private int mCurrentTop, mCurrentBottom;
    private int mCurrentTopS, mCurrentBottomS;
     ...
     * @param view
     */
    public void setHeader(View view, View header) {
        mHeader = view;
        mHeaderS = header;
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            mCurrentTopS = mInitTopS = mHeaderS.getTop();
            mCurrentBottomS = mInitBottomS = mHeaderS.getBottom();
        }
    ...
    private void doActionMove(MotionEvent event) {
        ...
        if (isMoving) {
            ...
            mCurrentTopS = (int) (mInitTopS + headerMoveHeight);
            mCurrentBottomS = (int) (mInitBottomS + headerMoveHeight);
            ...
            if (top <= headerBottom) {
                ...
                mHeaderS.layout(mHeaderS.getLeft(), mCurrentTopS, mHeaderS.getRight(), mCurrentBottomS);
            }
            //如果header移动很小的话，不执行动画
            if(headerMoveHeight > 15){
            	float scalef = event.getY()/mTouchDownY*SCROLL_RATIO*SCROLL_RATIO*SCROLL_RATIO*SCROLL_RATIO;
            	//System.out.println("headerMoveHeight="+headerMoveHeight+",scalef="+scalef);
                mHeader.setScaleX(1+scalef);
                mHeader.setScaleY(1+scalef);
            }
        }
    }
    ...
    private void rollBackAnimation() {
        ...
        mHeader.animate().scaleX(1.0f);
        mHeader.animate().scaleY(1.0f);

        mHeader.layout(mHeader.getLeft(), mInitTop, mHeader.getRight(), mInitBottom);


        TranslateAnimation tranAnimS = new TranslateAnimation(0, 0,
                Math.abs(mInitTopS - mCurrentTopS), 0);
        tranAnimS.setDuration(200);
        mHeaderS.startAnimation(tranAnimS);

        mHeaderS.layout(mHeaderS.getLeft(), mInitTopS, mHeaderS.getRight(), mInitBottomS);
